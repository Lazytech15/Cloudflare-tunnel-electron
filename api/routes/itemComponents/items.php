...
function handleFilterOptions() {
    ...
    $itemTypes = $db->query("SELECT DISTINCT item_type FROM itemsdb WHERE item_type IS NOT NULL ORDER BY item_type")->fetchAll(PDO::FETCH_COLUMN);
    $locations = $db->query("SELECT DISTINCT location FROM itemsdb WHERE location IS NOT NULL ORDER BY location")->fetchAll(PDO::FETCH_COLUMN);
    $itemStatuses = $db->query("SELECT DISTINCT item_status FROM itemsdb ORDER BY item_status")->fetchAll(PDO::FETCH_COLUMN);
    $suppliers = $db->query("SELECT DISTINCT supplier FROM itemsdb WHERE supplier IS NOT NULL ORDER BY supplier")->fetchAll(PDO::FETCH_COLUMN);
