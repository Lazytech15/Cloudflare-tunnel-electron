...
function handleStockQuantity($itemId) {
    ...
    $stmt = $db->prepare("UPDATE itemsdb SET in_qty = ?, out_qty = ?, balance = ? WHERE item_no = ?");
    $stmt->execute([$newInQty, $newOutQty, $newBalance, $itemId]);
...
function handleStockOut($itemId) {
    ...
    $stmt = $db->prepare("UPDATE itemsdb SET out_qty = ?, balance = ? WHERE item_no = ?");
    $stmt->execute([$newOutQty, $newBalance, $itemId]);
