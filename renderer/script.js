...
function setupEventListeners() {
  ...
  window.electronAPI.onServerLog((event, logData) => {
    ...
    addTerminalLog(level === 'error' ? 'error' : 'info', formattedMessage, timestamp)
  })
...
async function updateStatus() {
  try {
    ...
    updateStatusIndicator('database-status', status.databaseServerRunning, 'Database Server')
    updateStatusIndicator('tunnel-status', status.tunnelRunning, 'Cloudflare Tunnel')
...
function updateUrls(status) {
  ...
  if (status.databaseServerRunning) {
    // Local URL
    addUrlCard('Local Access', status.localUrl, 'Only accessible from this computer')
    
    // Network URL
    addUrlCard('Network Access', status.networkUrl, 'Accessible from devices on your network')
    
    // Tunnel URL (if available)
    if (status.tunnelUrl) {
      addUrlCard('Public Access', status.tunnelUrl, 'Accessible from anywhere on the internet', true)
    }
  } else {
    urlsContainer.innerHTML = '<p class="no-urls">Start the database server to see access URLs</p>'
  }
}
...
function updateButtons(status) {
  ...
  if (startServerBtn) {
    ...
    startServerBtn.textContent = status.databaseServerRunning ? 'Server Running' : 'Start Server'
  }
  
  // Start Tunnel button
  if (startTunnelBtn) {
    startTunnelBtn.disabled = !status.databaseServerRunning || status.tunnelRunning
    startTunnelBtn.textContent = status.tunnelRunning ? 'Tunnel Running' : 'Start Tunnel'
  }
  
  // Stop Services button
  if (stopServicesBtn) {
    stopServicesBtn.disabled = !status.databaseServerRunning && !status.tunnelRunning
...
function addTerminalLog(level, message, timestamp = null) {
  ...
  switch (level) {
    ...
    case 'info': default: color = '#74c0fc'; break
  }
